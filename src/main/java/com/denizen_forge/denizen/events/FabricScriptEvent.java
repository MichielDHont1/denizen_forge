...
import com.denizenscript.denizen.tags.ForgeTagContext;
import com.denizenscript.denizen.utilities.VanillaTagHelper;
import com.denizenscript.denizen.utilities.implementation.ForgeScriptEntryData;
...
import org.forge.Location;
import org.forge.Material;
import org.forge.World;
import org.forge.entity.*;
import org.forge.event.*;
import org.forge.event.inventory.InventoryType;
import org.forge.plugin.EventExecutor;
import org.forge.plugin.IllegalPluginAccessException;
import org.forge.plugin.Plugin;
import org.forge.plugin.RegisteredListener;
import org.forge.scheduler.ForgeRunnable;

import java.lang.reflect.Field;
import java.lang.reflect.Method;
import java.util.*;

public abstract class ForgeScriptEvent extends ScriptEvent {
    ...
    public boolean exactMatchesVehicle(String text) {
        ...
        if (EntityTag.matches(text)) {
            ...
            if (!Vehicle.class.isAssignableFrom(entity.getEntityType().getForgeEntityType().getEntityClass())) {
                return false;
            }
            return true;
        }
    ...
    public ForgeTagContext getTagContext(ScriptPath path) {
        ForgeTagContext context = (ForgeTagContext) getScriptEntryData().getTagContext().clone();
    ...
    public HashMap<EventPriority, ForgeScriptEvent> priorityHandlers;


    // <--[language]
    // @name Forge Event Priority
    // @group Script Events
    // @description
    // Script events that are backed by standard Forge events are able to control what underlying Forge event priority
    // they register as.
    ...
    // This can be done using the "forge_priority" switch.
    ...
    public ScriptEvent fire() {
        if (!Forge.isPrimaryThread()) {
            if (Debug.verbose) {
                Debug.log("Event is firing async: " + getName());
            }
            ForgeScriptEvent altEvent = (ForgeScriptEvent) clone();
            new ForgeRunnable() {
                @Override
                public void run() {
                    altEvent.fire();
    ...
    public void destroy() {
        if (priorityHandlers != null) {
            for (ForgeScriptEvent event : priorityHandlers.values()) {
                event.destroy();
            }
    ...
    public void initListener(Listener listener) {
        ...
        for (ScriptPath path : new ArrayList<>(eventPaths)) {
            String forgePriority = path.switches.get("forge_priority");
            if (forgePriority != null) {
                try {
                    EventPriority priority = EventPriority.valueOf(forgePriority.toUpperCase());
                    ForgeScriptEvent handler = priorityHandlers.get(priority);
                    if (handler == null) {
                        handler = (ForgeScriptEvent) clone();
                    ...
                    Debug.echoError("Invalid 'forge_priority' switch for event '" + path.event + "' in script '" + path.container.getName() + "'.");
                    Debug.echoError(ex);
                }
            }
        }
    ...
    public boolean runAutomaticPlayerSwitches(ScriptPath path) {
        ...
        ForgeScriptEntryData data = (ForgeScriptEntryData) getScriptEntryData();
        if (!data.hasPlayer()) {
            return false;
        }
    ...
    public static boolean tryItem(ItemTag item, String comparedto) {
        ...
        if (comparedto.equals("potion") && CoreUtilities.toLowerCase(item.getForgeMaterial().name()).contains("potion")) {
            return true;
        }
        MatchHelper matcher = createMatcher(comparedto);
    ...
    public static boolean tryEntity(EntityTag entity, String comparedto) {
        ...
        Entity bEntity = entity.getForgeEntity();
        comparedto = CoreUtilities.toLowerCase(comparedto);
