...
public class MaterialTag implements ObjectTag, Adjustable, FlaggableObject {
    ...
    //
    // -->
    static final IForgeRegistry<Block> BLOCKS = ForgeRegistries.BLOCKS;
    /**
    ...
    public static MaterialTag valueOf(String string, TagContext context) {
        ...
        if (string.equals("RANDOM")) {
            int rand = CoreUtilities.getRandom().nextInt(BLOCKS.getKeys().size());
            Iterator<ResourceLocation> iterator = BLOCKS.getKeys().iterator();
            for(int i = 0; i < rand; i++){
                iterator.next();
            }
            return new MaterialTag(BLOCKS.getValue(iterator.next()));
        }


        Block m = BLOCKS.getValue(new ResourceLocation(string));
        if (m != null) {
    ...
    public MaterialTag(BlockState block) {
    ...
    public MaterialTag(BlockSnapshot block) {
        this.modernData = block.getBlockData();
        this.material = modernData.getMaterial();
    }

    public MaterialTag(ResourceLocation resourceLocation) {
        this.modernData = block.getBlockData();
        this.material = modernData.getMaterial();
    }
    ...
    public boolean isStructure() {
        if (material.getPath().equals("CHORUS_PLANT") || material.getPath().equals("RED_MUSHROOM_BLOCK")  || material.getPath().equals("BROWN_MUSHROOM_BLOCK")) {
            return true;
        }
        return false;
    }
    ...
    public static void registerTags() {
        ...
        registerTag("advanced_matches", (attribute, object) -> {
            ...
            return new ElementTag(ForgeScriptEvent.tryMaterial(object, attribute.getContext(1)));
        });
