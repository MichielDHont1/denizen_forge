...
public class LocationTag extends org.bukkit.Location implements ObjectTag, Notable, Adjustable, FlaggableObject {
     ...
     */
    public LocationTag(PlayerEntity  player) {
        this(player.getWorld(), player.getPosX(), player.getPosY(), player.getPosZ(), location.getYaw(), location.getPitch());
    }
    public LocationTag(BlockPos  BlockPos, IWorld world) {
        this(world, BlockPos.getX(), BlockPos.getX(), BlockPos.getX(), 0, 0);
    }
    ...
    public static class FloodFiller {
        ...
        public void flood(LocationTag loc) {
            ...
            if (matcher == null ? loc.getBlock().getType() != requiredMaterial : !ForgeScriptEvent.tryMaterial(loc.getBlock().getType(), matcher)) {
                return;
            }
            result.add(loc);
                ...
                for (int x = -radiusInt; x <= radiusInt; x++) {
                    for (int z = -radiusInt; z <= radiusInt; z++) {
                        ...
                        if (Utilities.checkLocation(object, tstart.clone().add(x + 0.5, y + 0.5, z + 0.5), radius)) {
                            if (matcher == null || ForgeScriptEvent.tryMaterial(new LocationTag(tstart.clone().add(x, y, z)).getBlockTypeForTag(attribute), matcher)) {
